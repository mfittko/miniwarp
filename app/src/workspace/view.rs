...
use self::layout::compute_default_panel_widths;
...
impl TypedActionView for Workspace {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            AddDefaultTab => {
                self.handle_add_default_tab_action(ctx);
            }
            ...
            AddAmbientAgentTab => {
                self.handle_add_ambient_agent_tab_action(ctx);
            }
            AddAgentTab => {
                self.handle_add_agent_tab_action(ctx);
            }
            AddDockerSandboxTab => {
                self.handle_add_docker_sandbox_tab_action(ctx);
            }
            StartAgentOnboardingTutorial(tutorial) => {
                self.handle_start_agent_onboarding_tutorial_action(tutorial, ctx);
            }
